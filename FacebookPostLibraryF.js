...
class FacebookPostLibraryF {
    ...
    async checkStateAndContinue() {
        ...
        await this.delay(5000);
        await this.performAsyncTask();//任务完成后，发送事件给Automa
    ...
    async notifyCompletion() {
        const event = new CustomEvent('TaskCompleted', {
            detail: {
                task: true,
                message: 'cancleorderTaskCompleted Task has been completed',
                time: new Date(),
            }
        });
...
// 确保类在全局范围内可用
window.FacebookPostLibraryF = FacebookPostLibraryF;
// Export the class
